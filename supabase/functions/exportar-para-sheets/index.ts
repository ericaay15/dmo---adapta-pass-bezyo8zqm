...
// Translation from old sheet headers to new data model keys.
// Only needed for the 42 original columns (A-AP) that use old naming.
// New columns (AQ+) use keys directly — no translation needed.
...
function buildDataMap(
  ...
  // --- Scores from scoring_json (dynamic iteration) ---
  for (const [blocoId, bloco] of Object.entries(scoring?.blocos ?? {})) {
    const b = bloco as any
    map[`nota_${blocoId.toLowerCase()}`] = String(b?.nota ?? '')
    map[`classificacao_${blocoId.toLowerCase()}`] = b?.classificacao ?? ''
  }
  ...
  // --- Dynamic question answers (skip temas — handled explicitly below) ---
  ...
  // --- SEG block (normalize camelCase keys to snake_case) ---
  if (answersJson?.temasSelecionados != null) {
  ...
  if (answersJson?.temaOutros != null) {
...
async function getGoogleAccessToken() {
  console.log('Iniciando geracao de token do Google...')
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (sessionError || !session) {
      throw new Error(`Sessao nao encontrada: ${sessionError?.message || 'Erro desconhecido'}`)
    }
    ...
    if (companyError || !company) {
      throw new Error(`Empresa nao encontrada: ${companyError?.message || 'Erro desconhecido'}`)
    }
    ...
    if (!tabName) throw new Error('Aba nao encontrada')
    ...
    if (!headerRes.ok) throw new Error('Falha ao obter cabecalhos da planilha')
    ...
    console.error('Erro capturado no fluxo principal da funcao:', error)
